...
jobs=(huashancm121 huashancm130 huashanadds huashanaicp \
      lxanzu130    lxrelease130 lxanzu140   lxrelease140);
...
case $1 in
     ...
     ${jobs[6]})  $ServerDir/LegacyXperia140.sh "" "anzu" >$ScriptsJob 2>&1;;
     ${jobs[7]})  $ServerDir/LegacyXperia140.sh "synconly,legacyxperia" >$ScriptsJob 2>&1;
                  for device in ${DevicesLegacyXperia[@]}; do
                    $ServerDir/LegacyXperia140.sh "nosync,rmoutdevice,release" "$device" >>$ScriptsJob 2>&1;
                  done;;
     *) echo "";
